...
int IconCacheDatabaseManager::iconVersion()
{
	int icon_version = -1;
	QSqlQuery query(m_db);
	query.prepare(QString("SELECT icon_version FROM %1_metadata WHERE row=0").arg(m_tableBasename));
	if ( query.exec() ) {
		if ( query.first() )
			icon_version = query.value(0).toInt();
		query.finish();
	} else
		qmc2MainWindow->log(QMC2_LOG_FRONTEND, tr("WARNING: failed to fetch '%1' from icon cache database: query = '%2', error = '%3'").arg("icon_version").arg(query.lastQuery()).arg(query.lastError().text()));
	return icon_version;
}

void IconCacheDatabaseManager::setIconVersion(int icon_version)
{
	QSqlQuery query(m_db);
	query.prepare(QString("SELECT icon_version FROM %1_metadata WHERE row=0").arg(m_tableBasename));
	if ( query.exec() ) {
		if ( !query.next() ) {
			query.finish();
			query.prepare(QString("INSERT INTO %1_metadata (icon_version, row) VALUES (:icon_version, 0)").arg(m_tableBasename));
			query.bindValue(":icon_version", icon_version);
			if ( !query.exec() )
				qmc2MainWindow->log(QMC2_LOG_FRONTEND, tr("WARNING: failed to add '%1' to icon cache database: query = '%2', error = '%3'").arg("icon_version").arg(query.lastQuery()).arg(query.lastError().text()));
		} else {
			query.finish();
			query.prepare(QString("UPDATE %1_metadata SET icon_version=:icon_version WHERE row=0").arg(m_tableBasename));
			query.bindValue(":icon_version", icon_version);
			if ( !query.exec() )
				qmc2MainWindow->log(QMC2_LOG_FRONTEND, tr("WARNING: failed to update '%1' in icon cache database: query = '%2', error = '%3'").arg("icon_version").arg(query.lastQuery()).arg(query.lastError().text()));
		}
		query.finish();
	} else
		qmc2MainWindow->log(QMC2_LOG_FRONTEND, tr("WARNING: failed to fetch '%1' from icon cache database: query = '%2', error = '%3'").arg("icon_version").arg(query.lastQuery()).arg(query.lastError().text()));
...
qint64 IconCacheDatabaseManager::iconRowCount(bool reset)
{
...
qint64 IconCacheDatabaseManager::nextRowId(bool refreshRowIds)
{
	if ( refreshRowIds ) {
		m_rowIdList.clear();
		m_lastRowId = -1;
		...
		if ( query.exec(QString("SELECT rowid FROM %1").arg(m_tableBasename)) ) {
			...
			qmc2MainWindow->log(QMC2_LOG_FRONTEND, tr("WARNING: failed to fetch row IDs from icon cache database: query = '%1', error = '%2'").arg(query.lastQuery()).arg(query.lastError().text()));
			return -1;
		}
...
void IconCacheDatabaseManager::recreateDatabase()
{
	...
	query.finish();
	if ( !query.exec(QString("CREATE TABLE %1_metadata (row INTEGER PRIMARY KEY, emu_version TEXT, qmc2_version TEXT, icon_version INTEGER)").arg(m_tableBasename)) ) {
		qmc2MainWindow->log(QMC2_LOG_FRONTEND, tr("WARNING: failed to create icon cache database: query = '%1', error = '%2'").arg(query.lastQuery()).arg(query.lastError().text()));
		return;
	}
	if ( logActive() )
