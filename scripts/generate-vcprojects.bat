...
%QT_PATH%\bin\qmake.exe -tp vc SDL=%SDL_VERSION% QT+=phonon CONFIG+=warn_off CONFIG+=release INCLUDEPATH+=%SDL_INC_PATH% INCLUDEPATH+=%LIBARCHIVE_INC_PATH% LIBS+=%SDL_LIB_PATH% LIBS+=%SDLMAIN_LIB_PATH% LIBS+=%LIBARCHIVE_LIB_PATH% TARGET=qmc2-mame %QMC2_MAME_DEFINES% -o qmc2-mame.%VCPROJ_EXTENSION% qmc2.pro > NUL 2> NUL
...
%QT_PATH%\bin\qmake.exe -tp vc SDL=%SDL_VERSION% CONFIG+=warn_off CONFIG+=release INCLUDEPATH+=%SDL_INC_PATH% INCLUDEPATH+=%LIBARCHIVE_INC_PATH% LIBS+=%SDL_LIB_PATH% LIBS+=%SDLMAIN_LIB_PATH% LIBS+=%LIBARCHIVE_LIB_PATH% %QMC2_ARCADE_DEFINES% -o arcade\qmc2-arcade.%VCPROJ_EXTENSION% arcade\qmc2-arcade.pro > NUL 2> NUL
