  ...
  if ( listXMLCache.isOpen() ) {
    ...
    if ( xmlCacheOkay ) {
      ...
      while ( !ts.atEnd() || !readBuffer.isEmpty() ) {
        ...
        qmc2MainWindow->progressBarGamelist->setValue(qmc2MainWindow->progressBarGamelist->value() + gameCount);
        if ( endsWithNewLine )
          readBuffer.clear();
        else
...
void Gamelist::parse()
{
  ...
  switch ( qmc2SortCriteria ) {
    ...
    case QMC2_SORT_BY_ROM_STATE:
      sortCirtieria = QObject::tr("ROM state");
      break;
    case QMC2_SORT_BY_YEAR:
      sortCirtieria = QObject::tr("year");
      break;
